...
#else
#define BUFFER_LENGTH 128 // ESP32 default buffer size
#endif
#elif defined(ARDUINO_ARCH_AVR) || defined(ARDUINO_ARCH_MEGAAVR) || defined(__AVR__)
#define BUFFER_LENGTH 32 // Arduino AVR default
#elif defined(ARDUINO_ARCH_RP2040)
#define BUFFER_LENGTH 256 // Raspberry Pi Pico default
#else
#error "Unsupported platform! This library requires ESP32, ESP8266, Arduino AVR, or Raspberry Pi Pico."
#endif
#endif
...
inline size_t LiquidCrystal_I2C::writeString(const char *value, uint8_t length)
{
	...
	while (countChar < length) {
		...
		if (countI2C >=
		    (BUFFER_LENGTH >>
		     2)) { // if buffer will be exceeded on next characater, 4 bytes each character (4bit mode and EN High/Low)
...
void LiquidCrystal_I2C::pulseEnable(uint8_t _data)
{
	...
	// commands need > 37us to settle, next 2 Byte via I2C will need ~55us @ 400kHz (25us per Byte plus start/stop
	// condition) no delay is required as processing time is bigger than 37us
}
